...
def get_df(path_folder, positions):
    try:
        ...
        df_path = os.path.join(path_folder, f"Scouting_centiles{file_name}.csv")
...
if season_code:
    path_folder = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "csv", f"csv{season_code}", "scouting"))
    ...
    if positions:
        ...
        if df_radar.empty:
            ...
            df_global_path = os.path.join(path_folder, f"{file_name}")
            ...
            if selected_players:
                ...
                if 'Goalkeeper' in positions: 
                    df_metrics = pd.read_csv(os.path.join(path_folder, f"Scouting_metrics_gk.csv"))
                else:
                    df_metrics = pd.read_csv(os.path.join(path_folder, f"Scouting_metrics.csv"))
                ...
                for player in selected_players:
                    ...
                    df_adj_path = os.path.join(path_folder, f"{file_name}")
                    ...
                    for stat in valid_stats:
                        if stat in df_global.columns:
                            aggregated_values.append(df_global[stat].values[0])
                        else:
